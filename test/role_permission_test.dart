...
void main() {
  group('Тестирование матрицы ролей и прав (ПР5)', () {
    ...
    test('2. Администратор обладает всеми правами нижестоящих ролей', () {
      const admin = AppUser(
          id: 1, username: 'admin', fullName: 'Админ', role: Role.admin);
    ...
    test('4. Читатель не имеет доступа к редактированию фонда', () {
      const reader = AppUser(
          id: 3, username: 'reader', fullName: 'Читатель', role: Role.reader);
    ...
    test('5. Разбор ролей из серверного JSON', () {
      final user =
          AppUser.fromJson({'id': 10, 'username': 'boss', 'role': 'admin'});
      expect(user.role, Role.admin);
      final fallback = AppUser.fromJson(
          {'id': 11, 'username': 'unknown', 'role': 'something'});
